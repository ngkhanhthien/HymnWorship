...
def upload_media_file(
    ...
    """
    Upload a local binary file (image/mp3) to Firebase Cloud Storage.
...
def sync_hymns_to_firebase(hymns: List[Dict[str, Any]]) -> bool:
    """
    Sync all crawled hymns, sheet music, accompaniment and vocal audio to Firebase.
    """
    if not hymns:
    ...
    for i, hymn in enumerate(hymns, 1):
        ...
        logger.info(
            f"  [{i}/{len(hymns)}] Uploaded #{hymn_id} '{hymn.get('title')}' "
    ...
    json_storage_url = upload_media_file(bucket, local_json_path, json_storage_path, "application/json")
    if json_storage_url:
